...
public interface AdministrativnoOsobljeRepository extends JpaRepository<AdministrativnoOsoblje, Long> {
	...
	@Query("SELECT DISTINCT s, sy.godinaStudija.godina FROM Student s JOIN s.studentNaGodini sy JOIN s.pohadjanjePredmeta sa JOIN sa.realizacijaPredmeta sr WHERE sy.godinaStudija.id=?1 AND sa.konacnaOcena IS NOT NULL GROUP BY s.id HAVING (SUM(sr.predmet.espb))>=(48*(sy.godinaStudija.godina))")
