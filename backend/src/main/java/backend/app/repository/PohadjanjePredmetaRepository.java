...
public interface PohadjanjePredmetaRepository extends JpaRepository<PohadjanjePredmeta, Long> {
	...
	@Query("SELECT sa.student FROM PohadjanjePredmeta sa WHERE sa.konacnaOcena < 6 AND sa.realizacijaPredmeta.subject.id = ?1")
	...
	@Query("SELECT sa.konacnaOcena, sa.realizacijaPredmeta.predmet.naziv, sa.realizacijaPredmeta.godinaStudija.year, "
			+ "sa.realizacijaPredmeta.godinaStudija.studijskiProgram.naziv, er.bodovi, er.evaluacijaZnanja.vremePocetka, sa.realizacijaPredmeta.predmet.ects \n" + 
			...
			+ "AND sa.konacnaOcenaIS NOT NULL")
